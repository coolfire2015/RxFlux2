...
public class LoggerManager {
    ...
    public void logRxAction(String store, RxAction rxAction) {
        switch (LOG_LEVEL) {
            case FULL:
                Log.d(TAG, "Post RxAction to " + store + " -> " + rxAction.getType() + ", data: " + rxAction.getData().toString());
                break;
            case SIMPLIFY:
                int hash = rxAction.getType().hashCode() + rxAction.getData().hashCode();
                if (hash != lastActionHash) {
                    lastActionHash = hash;
                    Log.d(TAG, "Post RxAction -> " + rxAction.getType() + ", data: " + rxAction.getData().toString());
                }
                break;
            default:
                break;
    ...
    public void logRxStore(String store, RxStoreChange storeChange) {
        switch (LOG_LEVEL) {
            case FULL:
            case SIMPLIFY:
                Log.d(TAG, "Post RxStore change to " + store + " -> " + storeChange.getStoreId() + " action: " + storeChange.getRxAction().toString());
                break;
            case NONE:
                break;
        }
